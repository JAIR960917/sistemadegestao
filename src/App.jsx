...
Usuário: {currentUser === "client" ? "Cliente" : "Admin"}
<button
...
(!newRequest.servico || !newRequest.descricao) {
...
{selectedOrder.produtos.map((p, idx) => (
• {p.nome} - R {p.preco}</li> ))} </ul> <p className="font-semibold">💰 Total: R {selectedOrder.total}
<button
onClick={() => setSelectedOrder(null)}
...
!newService.descricao || !newService.valor) {
alert("Preencha todos os campos!");
return;
}
  ...
  <div className="bg-white p-4 rounded shadow mb-6">
    ...
    <input
      ...
      onChange={(e) => editingService 
        ? setEditingService({...editingService, nome: 
e.target.value})
...
mb-4">📝 Gerenciar Solicitações de Serviço
...
Cliente: {selectedRequest.cliente}
Serviço: {selectedRequest.servico}
Data: {selectedRequest.data}
Status: {selectedRequest.status}
Valor: R$ {selectedRequest.valor}
...
/>
    ...
    <div className="mb-4">
      ...
      <input
        ...
        onChange={(e) =>
          setSystemConfig({ 
...systemConfig, secondaryColor: e.target.value })
}
className="border rounded"
/>
    <div className="mb-4">
